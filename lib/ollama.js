...
// Класифікація одного повідомлення клієнта відносно списку FAQ.
// Модель повертає ЛИШЕ номер відповідного питання (або null) — сам текст
// відповіді генерувати не потрібно, це усуває ризик "порожньої" чи спотвореної
// відповіді на питання зі складним текстом (URL, списки тощо).
// Повертає matchedId (число або null) або null при помилці/таймауті запиту.
async function classifyMessage({ config, systemPrompt, userText }) {
  try {
    const raw = await callOllama({
      url: config.ollamaUrl,
      model: config.chatModel,
    ...
    const parsed = JSON.parse(raw);
    if (parsed.matchedId === null || Number.isInteger(parsed.matchedId)) {
      return parsed.matchedId;
    }
    console.error('[ollama] Класифікатор повернув не за форматом:', raw);
    return null;
  } catch (err) {
    console.error('[ollama] Помилка/таймаут класифікації:', err.message);
    return null;
  }
}
...
module.exports = { askOllama, classifyMessage, analyzeDailyLog };